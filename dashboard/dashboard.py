...
def load_data():
    ...
    customer_feat   = pd.read_csv("customer_segment_mapping.csv")   # ← fixed filename
    ...
    seg_lookup = (
        customer_feat[["customer_id", "segment"]]                   # ← fixed column name
        .drop_duplicates("customer_id")                             # ← removed redundant rename
...
if st.sidebar.button("🔄 Reset "):
    for k in ["f_date_from", "f_date_to", "f_countries", "f_categories", "f_segments"]:
...
if not country_filter:
    st.warning("⚠️ No country selected. Please select at least one.")
    st.stop()
if not category_filter:
    st.warning("⚠️ No category selected. Please select at least one.")
    st.stop()
if not segment_filter:
    st.warning("⚠️ No segment selected. Please select at least one.")
    st.stop()
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    ...
    "Customer Intelligence", "Top Products", "Business Insights",
])
...
with tab2:
    ...
    # --------------------------------------------------
    # 🔥 CUSTOMER TABLE (BASED ON SEGMENT DISTRIBUTION)
    # --------------------------------------------------

    st.subheader("Customers by Selected Segment")

    customers_segment = seg_lookup[
        seg_lookup["customer_id"].isin(active_cust_ids)
    ]

    customers_segment = pd.merge(
        customers_segment,
        customers_df,
        on="customer_id",
        how="left"
    ...
    # --------------------------------------------------
    # 🔥 CUSTOMER TABLE (BASED ON REVENUE CONTRIBUTION)
    # --------------------------------------------------
    ...
    st.dataframe(customer_revenue[cols_rev], use_container_width=True)
        ...
        # --------------------------------------------------
    # 🔁 REPEAT CUSTOMERS (FILTERED BY DATE RANGE)
    # --------------------------------------------------

    st.divider()
    st.subheader("Repeated Customers ")

    # STEP 1: Count orders per customer (AFTER FILTERING)
    repeat_orders = (
    ...
    # STEP 2: Filter repeat customers (>1 orders)
    repeat_customers = repeat_orders[repeat_orders["order_count"] > 1]

    # STEP 3: Add segment info
    repeat_customers = pd.merge(
        repeat_customers,
        seg_lookup,
        on="customer_id",
        how="left"
    ...
    # STEP 5: Create name if not exists
    ...
    # STEP 7: Sort by most frequent buyers
...
with tab3:
    ...
    # --------------------------------------------------
    # SELECT CUSTOMER (IMPORTANT — THIS FIXES YOUR ERROR)
    # --------------------------------------------------
    ...
    customer_info = customers_df[
        customers_df["customer_id"] == selected_customer
    ]

    rfm_df = pd.read_csv("customer_segment_mapping.csv")
    rfm_info = rfm_df[
        rfm_df["customer_id"] == selected_customer
    ]
    ...
    # --------------------------------------------------
    # CUSTOMER DETAILS (WITHOUT signup_date)
    # --------------------------------------------------
    ...
    recs = recommendations[
        recommendations["customer_id"] == selected_customer
    ]
...
with tab4:
    ...
    fig_cat = px.bar(
        cat_rev, x="category", y="revenue", color="category",
